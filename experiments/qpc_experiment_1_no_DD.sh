...
OUTPUT_DIR="experiments/experiment_1/QCP_no_DD" # Replace with your desired output file
...
for INPUT_FILE in "$INPUT_DIR"/*.mzn; do
  if [ -f "$INPUT_FILE" ]; then
    ...
    minizinc --time-limit 1200000 --solver "minizinc/pumpkin.msc" $INPUT_FILE -s > $OUTPUT_FILE
