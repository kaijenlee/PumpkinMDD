...
OUTPUT_DIR="experiments/${OUTPUT_FOLDER}/market_split_SRV_Branch_first" # Replace with your desired output file
SKIP=false
if [ -n "$3" ]; then
    SKIP=false
fi
...
for INPUT_FILE in "$INPUT_DIR"/*.dzn; do
  if [ -f "$INPUT_FILE" ]; then
    ...
    minizinc --time-limit 3600000 --solver "minizinc/pumpkin.msc" $MODEL $INPUT_FILE -s --dd-enable --dd-srv-enable --branch-srv-first --dd-max-width $DD_WIDTH> $OUTPUT_FILE
