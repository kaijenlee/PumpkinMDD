...
for INPUT_FILE in "$INPUT_DIR"/*.dzn; do
  if [ -f "$INPUT_FILE" ]; then
    ...
    minizinc --time-limit 600000 --solver "minizinc/pumpkin.msc" $MODEL $INPUT_FILE -s --dd-enable --dd-srv-enable --dd-max-width $DD_WIDTH> $OUTPUT_FILE

  fi
done

echo "All files processed. Outputs saved to $OUTPUT_DIR"
python3 "experiments/extract_fzn_stats.py" $OUTPUT_DIR "experiments/${OUTPUT_FOLDER}/marketsplit_srv.csv"
