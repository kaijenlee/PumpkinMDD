...
impl<Var: IntegerVariable + 'static> NotEqualConstraint<Var> {
    fn to_propagator(self) -> SingleInferencePropagatorArgs<LinearNotEqualPropagatorArgs<Var>> {
        ...
        SingleInferencePropagatorArgs {
            wrapped_args: LinearNotEqualPropagatorArgs {
                terms: terms.into(),
                rhs,
            },
            constraint_tag,
...
{
    fn post(self, solver: &mut Solver) -> Result<(), ConstraintOperationError> {
        self.to_propagator().post(solver)
    }
    ...
    fn implied_by(
        ...
        self.to_propagator().implied_by(solver, reification_literal)
    }
}
